...
export function getRelatedSizeCards(currentSize = "", { type = "", limit = 6 } = {}) {
  ...
  const commercialAnchors = ["295/75R22.5", "11R22.5", "12R22.5", "275/80R22.5", "285/75R24.5", "445/50R22.5"];
  const relatedSizes = getRelatedSizes({ currentSize, sizes: tireSizes, type: pageType, limit: Math.max(limit, 10) });
  const anchoredSizes = pageType === "commercial"
    ? [...commercialAnchors, ...relatedSizes]
    : relatedSizes;

  return [...new Set(anchoredSizes)]
    .filter((size) => sizeToSlug(size) !== sizeToSlug(currentSize))
    .slice(0, limit)
    .map((size) => ({
      size,
      href: `/tires/${sizeToSlug(size)}`,
